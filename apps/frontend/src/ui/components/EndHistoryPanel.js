...
export function EndHistoryPanel({ history, bestScore, playerName }) {
  return h('div', { class: 'lg:col-span-5 flex flex-col h-full min-h-[600px] lg:min-h-0 p-1' },
    h('div', { class: 'glass-panel p-6 sm:p-10 rounded-[2.5rem] h-full flex flex-col border border-white/5 overflow-hidden' },
      ...
      h('div', { class: 'grid grid-cols-2 gap-4 mb-8 shrink-0' },
        h('div', { class: 'p-4 sm:p-6 rounded-[1.5rem] sm:rounded-[2rem] bg-white/[0.03] border border-white/5 flex flex-col justify-center' },
          h('div', { class: 'flex items-center gap-3 mb-2' },
            h('div', { class: 'icon-wallet text-slate-500 w-3 h-3 sm:w-4 sm:h-4' }),
            h('p', { class: 'text-[8px] sm:text-[9px] font-black text-slate-500 uppercase tracking-[0.2em]' }, TEXT.end.lifetimeEarnings)
          ),
          h('p', { class: 'text-xl sm:text-2xl font-black text-white font-mono leading-none' }, formatScore(history.reduce((sum, entry) => sum + (entry.score || 0), 0)))
        ),
        h('div', { class: 'p-4 sm:p-6 rounded-[1.5rem] sm:rounded-[2rem] bg-white/[0.03] border border-white/5 flex flex-col justify-center' },
          h('div', { class: 'flex items-center gap-3 mb-2' },
              ...
              h('div', { class: 'flex items-center gap-6' },
                ...
                h('div', { class: 'flex flex-col' },
                  h('div', { class: 'flex items-center gap-3' },
                    ...
                    isFirst && h('span', { class: 'hidden lg:inline text-[8px] font-black bg-emerald-500 text-white px-2 py-1 rounded-md uppercase tracking-tighter' }, TEXT.end.newTag)
                  )
                )
              ),
