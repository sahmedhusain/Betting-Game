...
export function EndSummaryPanel({ score, bestScore, comment, onPlayAgain }) {
  return h('div', { class: 'lg:col-span-7 flex flex-col h-full min-h-0 p-1' },
    h('div', { class: 'glass-panel p-8 sm:p-12 rounded-[3rem] border border-white/5 relative overflow-hidden h-full flex flex-col justify-center min-h-0' },
      // Glow background
      ...
      h('div', { class: 'relative z-10' },
        h('div', { class: 'mb-8 sm:mb-12' },
          ...
          h('div', { class: 'flex flex-col' },
            h('span', { class: 'text-5xl sm:text-[90px] font-black font-outfit tracking-tighter leading-[0.8] text-white opacity-40' }, 'CAUGHT'),
            h('span', { class: 'text-5xl sm:text-[90px] font-black font-outfit tracking-tighter leading-[0.8] text-white' }, 'UP!')
          )
        ),

        h('div', { class: 'py-8 sm:py-10 border-y border-white/5 mb-8 sm:mb-10' },
          ...
          h('div', { class: 'flex items-center gap-4 mb-3' },
            h('div', { class: 'w-1.5 h-12 bg-emerald-500 rounded-full shadow-[0_0_15px_rgba(16,185,129,0.4)]' }),
            h('h2', { class: 'text-6xl sm:text-8xl font-black text-white font-outfit tracking-tighter' },
              formatScore(score)
            )
          ),
          h('p', { class: 'text-emerald-400 text-[10px] font-black uppercase tracking-[0.3em]' }, comment)
        ),

        h('div', { class: 'flex flex-col sm:flex-row items-center gap-4' },
          h('button', {
